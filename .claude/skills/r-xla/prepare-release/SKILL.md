---
name: r-xla-prepare-release
description: >
  Prepare a release PR for a single R package. Use when the user says "prepare a
  release", "release this package", "bump version", "cut a release", or wants to
  publish a new version of the current R package. Must be run from inside the
  package directory (contains a DESCRIPTION file).
user_invocable: true
tools: Read, Edit, Glob, Grep, Bash, Write, AskUserQuestion, Skill
---

# Prepare R Package Release

Your role is to prepare a release PR for a single R package. You do NOT merge
the PR — your job is done once the PR is created and CI is passing.
...
The PR is ready for review and merge. Merging, tagging, and bumping to
development version are left to the user.

## Important Rules

1. **Always create a release branch** — never commit directly to main
2. **Never skip CI** — wait for checks to pass before finishing
3. **Do not merge** — your job ends when the PR is created and CI passes
4. **Use `/pr-create`** for PR creation and CI monitoring — don't reinvent it
